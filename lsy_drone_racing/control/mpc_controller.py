...
def export_quadrotor_ode_model() -> AcadosModel:
    ...
    for i in range(4) :
        xmin = obs_params[6*i + 0]
        ymin = obs_params[6*i + 1]
        zmin = obs_params[6*i + 2]
        xmax = obs_params[6*i + 3]
        ymax = obs_params[6*i + 4]
        zmax = obs_params[6*i + 5]

        pen_x = fmax(0, xmin - px) + fmax(0, px - xmax)
        pen_y = fmax(0, ymin - py) + fmax(0, py - ymax)
        pen_z = fmax(0, zmin - pz) + fmax(0, pz - zmax)

        penalty += pen_x*pen_x + pen_y*pen_y + pen_z*pen_z
    
    for i in range(16) :
        xmin = gate_params[6*i + 0]
        ymin = gate_params[6*i + 1]
        zmin = gate_params[6*i + 2]
        xmax = gate_params[6*i + 3]
        ymax = gate_params[6*i + 4]
        zmax = gate_params[6*i + 5]
    ...
    # === Final penalty cost ===
    penalty_weight = 1e4  # Tune based on solver behavior
    ...
    model.cost_expr_ext_cost = penalty_cost
    model.x = states
    model.u = inputs
    model.p = p
